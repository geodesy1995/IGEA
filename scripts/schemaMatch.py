...
INPUT_FILE = DATA_DIR + 'nca dataset.tsv'
OUTPUT_FILE = DATA_DIR + 'predicted classes.tsv'

PREDICTION_THRESHOLD = config.getfloat('nca', 'prediction_threshold')
...
class SchemaModel:
    ...
    def define_discriminator(self, no_inputs, no_outputs):
        ...
        latent_rep = Dense(LATENT_SPACE, activation='relu', name = 'latentRep')(X_1)

        # KG classfication
        fc_1 = Dense(LATENT_SPACE, activation='relu', name = 'layer3')(latent_rep)
        fc_2 = Dense(LATENT_SPACE , activation='relu', name = 'layer4')(fc_1)
...
hist = m.model.fit(x=x_train, y=[y_train, adverse_train], epochs=NUM_EPOCHS, shuffle=True, verbose=TRAIN_VERBOSE)
...
print(f'-predicting potential class matches with threshold {PREDICTION_THRESHOLD}')
matches = getMatches()
matchDF = pd.DataFrame(matches, columns=['tag', 'cls', 'value'])
matchDF = matchDF[matchDF['value'] > PREDICTION_THRESHOLD]
print(f'-found {len(matchDF)} predicted class matches')
matchDF.to_csv(OUTPUT_FILE, sep='\t', encoding='utf-8', index=False)
